# Micropython for Windows
I cloned https://github.com/micropython/micropython for this build!
# How to install
Install the exe and drag to C:\micropython
# How to use
Either open C:\micropython (or where you dragged the exe) with cmd.exe and run micropython.exe<br>
OR<br>
Add to PATH using sysdm.cpl<br>
[![Tutorial for adding to PATH](https://chris75hun.hu/github/micropythonbuild/tutorial.png)](https://youtu.be/WDDIUmf41-A)
